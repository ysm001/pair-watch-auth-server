...
module.exports = (function() {
   ...
   * @param {Integer} timeout レスポンス待機時間 
   */
  var _findNearPermissionHolders = function(socket, permissionHolders, callback, timeout) {
    ...
    var tasks = [
      ...
      function(distances, next) {
        ...
        var nearDistances = distances.filter(function(distance) {return distance.distance == 'immediate'})
        var nearUserIds = nearDistances.map(function(distance) {
          if (distance['source-id'] == userId) return distance['target-id'];
          else if (distance['target-id'] == userId) return distance['source-id'];
          else return null;
        });
